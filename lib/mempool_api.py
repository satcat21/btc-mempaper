...
class MempoolAPI:
    ...
    def get_recent_block_fees(self):
        """Median fee of each of the ~15 most recent blocks.

        Returns:
            list[dict]: [{'height': int, 'timestamp': int, 'median_fee': float}, ...]
                        newest first, or [] on failure.

        /v1/blocks is part of every mempool instance, indexed or not, so this is
        the one history source that always answers. It only reaches back about
        two and a half hours, which is enough to warm a cold baseline but not to
        be one.
        """
        try:
            url = f"{self.base_url}/v1/blocks"
            response = self._get(url, 15)
            response.raise_for_status()
            blocks = response.json()
        except (requests.RequestException, ValueError) as e:
            print(f"Error fetching recent block fees: {e}")
            return []
        ...
        for block in blocks if isinstance(blocks, list) else []:
            ...
            median = extras.get('medianFee')
            height = block.get('height')
            if median is None or height is None:
                continue
            try:
                out.append({'height': int(height),
                            'timestamp': int(block.get('timestamp') or 0),
                            'median_fee': float(median)})
            except (TypeError, ValueError):
                continue
        return out

    def get_historical_fee_rates(self, period="1m"):
        """Per-block fee-rate percentiles over a period, for baseline backfill.

        Args:
            period (str): mempool time window - 24h, 3d, 1w, 1m, 3m, 6m, 1y, 2y, 3y.

        Returns:
            list[dict]: [{'height': int, 'timestamp': int, 'median_fee': float}, ...]
                        or [] when the endpoint is unavailable.

        This lives in mempool's mining module, which only exists when the
        instance runs with block indexing enabled. A self-hosted mempool without
        indexing answers 404 here, which is normal and not worth logging as an
        error - the caller falls back to accumulating history locally.
        """
        try:
            url = f"{self.base_url}/v1/mining/blocks/fee-rates/{period}"
            response = self._get(url, 20)
            if response.status_code == 404:
                return []          # no mining module; expected, not an error
            response.raise_for_status()
            rows = response.json()
        except (requests.RequestException, ValueError) as e:
            print(f"Fee-rate history unavailable ({period}): {e}")
            return []
        ...
        for row in rows if isinstance(rows, list) else []:
            if not isinstance(row, dict):
                continue
            # avgFee_50 is the 50th percentile fee rate in that block, i.e. the
            # median a transaction actually had to pay to get in.
            median = row.get('avgFee_50')
            height = row.get('avgHeight')
            if median is None or height is None:
                continue
            try:
                out.append({'height': int(height),
