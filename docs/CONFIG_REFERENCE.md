...
| `tor_auto_restart` | Boolean | `true` | `true`, `false` | Last rung of the Tor recovery ladder: after ~35 minutes in which nothing has reached the mempool host over Tor, let mempaper restart the tor service. `install.sh` writes the sudoers rule it needs and enables the control socket the rung above it uses; where the rule is missing, mempaper logs the exact line and skips the rung. Set to `false` if the device runs other services that a tor restart would interrupt. |
